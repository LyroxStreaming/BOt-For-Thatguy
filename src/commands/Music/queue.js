...
module.exports = {
  ...
  execute: async (message, args, client, prefix, player) => {
    ...
    if (!tracks.length)
      return message.reply({
        embeds: [new EmbedBuilder()
          .setAuthor({name: `Queue for ${message.guild.name}  -  [ ${player.queue.length} Tracks ]`, iconURL: message.guild.iconURL({dynamic: true})})
    ...
    return collectormsg2(client, message, embeds, message.author);
  },
};
